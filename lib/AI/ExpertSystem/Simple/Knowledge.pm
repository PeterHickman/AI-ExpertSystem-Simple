...
sub diagnostic_has_question {
	my ($self) = @_;

        die "Knowledge->diagnostic_has_question() takes no arguments" if scalar(@_) != 1;

	return defined($self->{_question});
}
...
=item diagnostic_has_question( )

Returns true if the attribute has a question to ask the user.
...
=item Knowledge->diagnostic_has_question() takes no arguments
