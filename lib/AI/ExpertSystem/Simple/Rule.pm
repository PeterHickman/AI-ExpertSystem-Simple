...
sub diagnostic_usage {
	...
	die "Rule->diagnostic_usage() takes no arguments" if(scalar(@_) != 1);

	if($self->{_state} eq 'active') {
		foreach my $condition (keys %{$self->{_tested}}) {
			if($self->{_tested}->{$condition} == 1) {
				return 'active';
			}
		}
		return 'unused';
...
A utility function to return the state of the rules. Bit of a hack that may well be removed
...
When the method is called it requires no arguments. This message is given if arguments were supplied.
...
When the method is called it requires no arguments. This message is given if less arguments were supplied.
...
=item Rule->diagnostic_usage() takes no arguments
