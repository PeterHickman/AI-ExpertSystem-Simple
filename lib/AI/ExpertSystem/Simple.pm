...
sub load {
	...
	if(-f $filename) {
		...
		$self->_add_to_log( "There are " . $self->diagnostic_rules() . " rules" );
		$self->_add_to_log( "There are " . $self->diagnostic_attributes() . " attributes" );
		$self->_add_to_log( "There are " . $self->diagnostic_questions() . " questions" );
		$self->_add_to_log( "The goal attibutes is " . $self->{_goal}->name() );
		$self->_add_to_log( "The goal can be set by " . $self->diagnostic_outcomes() . " rules" );
		$self->_add_to_log( "There are " . $self->diagnostic_outcomes_distinct() . " distinct goal values" );
		return 1;
	} else {
...
sub explain {
	...
	my $x = "The goal '$name' was set to '$value' by rule '$rule'";
	$self->_add_to_log( $x );

	my @processed_rules = ( $rule );
...
sub diagnostic_outcomes {
	my ( $self ) = @_;

	die "Simple->diagnostic_outcomes() takes no arguments" if scalar(@_) != 1;

	my $result = 0;
	my $goal = $self->{_goal}->name();
...
sub diagnostic_outcomes_distinct {
	my ( $self ) = @_;

	die "Simple->diagnostic_outcomes_distinct() takes no arguments" if scalar(@_) != 1;
	...
	return scalar keys %answers;
}

sub diagnostic_rules {
	my ( $self ) = @_;

	die "Simple->diagnostic_rules() takes no arguments" if scalar(@_) != 1;

	return $self->{_number_of_rules};
}

sub diagnostic_attributes {
	my ( $self ) = @_;

	die "Simple->diagnostic_attributes() takes no arguments" if scalar(@_) != 1;

	return $self->{_number_of_attributes};
}

sub diagnostic_questions {
	my ( $self ) = @_;

	die "Simple->diagnostic_questions() takes no arguments" if scalar(@_) != 1;

	return $self->{_number_of_questions};
}

sub diagnostic_usage {
	my ( $self ) = @_;

	die "Simple->diagnostic_usage() takes no arguments" if scalar(@_) != 1;

	my %usage;

	foreach my $key (keys(%{$self->{_rules}})) {
		$usage{$key} = $self->{_rules}->{$key}->diagnostic_usage();
	}
...
sub expose_rules {
	...
	die "Simple->expose_rules() takes no arguments" if scalar(@_) != 1;

	return %{$self->{_rules}};
}

sub expose_knowledge {
	my ( $self ) = @_;

	die "Simple->expose_knowledge() takes no arguments" if scalar(@_) != 1;
...
sub expose_goal {
	...
	die "Simple->expose_goal() takes no arguments" if scalar(@_) != 1;
...
=item diagnostic_outcomes( )

Returns the number of rules that set the goal attribute

=item diagnostic_outcomes_distinct( )

Returns the number of distinct values that the rules can set the goal attribute to

=item diagnostic_rules( )

Returns the number of rules in the knowledge base

=item diagnostic_attributes( )

Returns the number of attributes in the knowledge base

=item diagnostic_questions( )

Returns the number of questions in the knowledge base

=item diagnostic_usage( )

Returns a hash of rules names and their useage

=item expose_rules( )

Return access to the rules hash

=item expose_knowledge( )

Return access to the knowledge hash

=item expose_goal( )

Return access to the goal
...
=item Simple->diagnostic_outcomes() takes no arguments

When the method is called it requires no arguments. This message is given if 
some arguments were supplied.

=item Simple->diagnostic_outcomes_distinct() takes no arguments

When the method is called it requires no arguments. This message is given if 
some arguments were supplied.

=item Simple->diagnostic_rules() takes no arguments

When the method is called it requires no arguments. This message is given if 
some arguments were supplied.

=item Simple->diagnostic_attributes() takes no arguments

When the method is called it requires no arguments. This message is given if 
some arguments were supplied.

=item Simple->diagnostic_questions() takes no arguments

When the method is called it requires no arguments. This message is given if 
some arguments were supplied.

=item Simple->diagnostic_usage() takes no arguments

When the method is called it requires no arguments. This message is given if 
some arguments were supplied.

=item Simple->expose_rules() takes no arguments

When the method is called it requires no arguments. This message is given if 
some arguments were supplied.

=item Simple->expose_knowledge() takes no arguments

When the method is called it requires no arguments. This message is given if 
some arguments were supplied.

=item Simple->expose_goal() takes no arguments
