...
is($x->given('b', 2), 'active', 'Should have no effect');
...
like($@, qr/^Rule->diagnostic_usage\(\) takes no arguments /, 'Too many arguments');

is( $x->diagnostic_usage(), 'unused', 'Should be unused');

$x->given('a', 1);

is( $x->diagnostic_usage(), 'active', 'Should be active');

$x->given('b', 2);

is( $x->diagnostic_usage(), 'completed', 'Should be completed');

$x->reset();

$x->given('a', 9);

is( $x->diagnostic_usage(), 'invalid', 'Should be invalid');
